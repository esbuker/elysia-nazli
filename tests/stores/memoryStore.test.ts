...
describe('MemoryRateLimitStore - core counting', () => {
  it('reports remaining counts decrementing toward zero', () => {
    ...
    const now = Date.now()
    const a = store.hit({ key: 'k', limit: 3, window: 1000, cost: 1, now })
    const b = store.hit({ key: 'k', limit: 3, window: 1000, cost: 1, now })
    const c = store.hit({ key: 'k', limit: 3, window: 1000, cost: 1, now })
    const d = store.hit({ key: 'k', limit: 3, window: 1000, cost: 1, now })
  ...
  it('charges the configured cost per hit', () => {
    const store = new MemoryRateLimitStore()
    const now = Date.now()
    const r1 = store.hit({ key: 'k', limit: 5, window: 1000, cost: 2, now })
    const r2 = store.hit({ key: 'k', limit: 5, window: 1000, cost: 2, now })
    const r3 = store.hit({ key: 'k', limit: 5, window: 1000, cost: 2, now })
  ...
  it('isolates counters per key', () => {
    const store = new MemoryRateLimitStore()
    const now = Date.now()
    const a1 = store.hit({ key: 'a', limit: 1, window: 1000, cost: 1, now })
    const b1 = store.hit({ key: 'b', limit: 1, window: 1000, cost: 1, now })
  ...
  it('rolls over when window expires (resetAt <= now)', () => {
    ...
    const a = store.hit({ key: 'k', limit: 1, window: 1000, cost: 1, now: start })
    const b = store.hit({ key: 'k', limit: 1, window: 1000, cost: 1, now: start + 500 })
    const c = store.hit({ key: 'k', limit: 1, window: 1000, cost: 1, now: start + 1000 })
...
describe('MemoryRateLimitStore - ban window', () => {
  it('does not arm a ban when ban is 0', () => {
    const store = new MemoryRateLimitStore()
    const now = Date.now()

    store.hit({ key: 'k', limit: 1, window: 1000, cost: 1, ban: 0, now })
    const r = store.hit({ key: 'k', limit: 1, window: 1000, cost: 1, ban: 0, now })

    expect(r.blocked).toBeTrue()
    expect(r.banUntil).toBeUndefined()
    expect(r.retryAfter).toBe(1000)
  })
  ...
  it('preserves an active ban across window rollover', () => {
    ...
    store.hit({ key: 'k', limit: 1, window: 1000, cost: 1, ban: 10_000, now: start })
    store.hit({ key: 'k', limit: 1, window: 1000, cost: 1, ban: 10_000, now: start })
  ...
  it('reports retryAfter as max(reset, ban) - now', () => {
    const store = new MemoryRateLimitStore()
    const start = 1_000_000

    store.hit({ key: 'k', limit: 1, window: 500, cost: 1, ban: 5000, now: start })
    const breach = store.hit({ key: 'k', limit: 1, window: 500, cost: 1, ban: 5000, now: start })

    expect(breach.retryAfter).toBe(5000)
  })
})
...
describe('MemoryRateLimitStore - cleanup', () => {
  it('removes only fully expired entries', () => {
    ...
    store.hit({ key: 'live', limit: 5, window: 10_000, cost: 1, now: start })
  ...
  it('does not remove entries with a future ban even if window expired', () => {
    ...
    store.hit({ key: 'k', limit: 1, window: 100, cost: 1, ban: 60_000, now: start })
    store.hit({ key: 'k', limit: 1, window: 100, cost: 1, ban: 60_000, now: start })
...
describe('MemoryRateLimitStore - maxEntries cap', () => {
  ...
  it('keeps Map size <= maxEntries via eviction', () => {
    ...
    for (let i = 0; i < 50; i++) {
      store.hit({ key: `key-${i}`, limit: 100, window: 60_000, cost: 1, now })
    }
  ...
  it('evicts expired entries before evicting live ones (cardinality DoS guard)', () => {
    ...
    for (let i = 0; i < 3; i++) {
      store.hit({ key: `old-${i}`, limit: 5, window: 100, cost: 1, now: start })
    }

    const future = start + 1000

    store.hit({ key: 'fresh-1', limit: 5, window: 60_000, cost: 1, now: future })
    store.hit({ key: 'fresh-2', limit: 5, window: 60_000, cost: 1, now: future })

    expect(store.size()).toBeLessThanOrEqual(3)

    const fresh1 = store.hit({
      key: 'fresh-1',
      limit: 5,
      window: 60_000,
      cost: 1,
    ...
    const old0 = store.hit({ key: 'old-0', limit: 5, window: 60_000, cost: 1, now: future + 1 })
  ...
  it('falls back to evicting oldest insertion-order keys when nothing is expired', () => {
    ...
    store.hit({ key: 'a', limit: 5, window: 60_000, cost: 1, now })
    store.hit({ key: 'b', limit: 5, window: 60_000, cost: 1, now })
    store.hit({ key: 'c', limit: 5, window: 60_000, cost: 1, now })
    store.hit({ key: 'd', limit: 5, window: 60_000, cost: 1, now })

    expect(store.size()).toBe(3)

    const aAgain = store.hit({ key: 'a', limit: 5, window: 60_000, cost: 1, now })

    expect(aAgain.count).toBe(1)
    const dAgain = store.hit({ key: 'd', limit: 5, window: 60_000, cost: 1, now })
  ...
  it('refreshes recency: a re-hit key moves to the end of insertion order', () => {
    ...
    const aResult = store.hit({ key: 'a', limit: 5, window: 60_000, cost: 1, now })

    expect(aResult.count).toBe(3)

    const probe = new MemoryRateLimitStore({ maxEntries: 2 })

    probe.hit({ key: 'a', limit: 5, window: 60_000, cost: 1, now })
    probe.hit({ key: 'b', limit: 5, window: 60_000, cost: 1, now })
    probe.hit({ key: 'a', limit: 5, window: 60_000, cost: 1, now })
    probe.hit({ key: 'c', limit: 5, window: 60_000, cost: 1, now })
    expect(probe.hit({ key: 'b', limit: 5, window: 60_000, cost: 1, now }).count).toBe(1)
  })
  ...
  it('disables the cap when maxEntries === 0', () => {
    ...
    for (let i = 0; i < 1000; i++) {
      store.hit({ key: `k-${i}`, limit: 100, window: 60_000, cost: 1, now })
    }
